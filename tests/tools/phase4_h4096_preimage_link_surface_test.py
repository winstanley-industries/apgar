...
class Phase4H4096PreimageLinkSurfaceTest(unittest.TestCase):
    def test_final_binaries_contain_no_execution_capability(self) -> None:
        ...
        for rootpath in _ROOTPATHS[1:]:
            ...
            with self.subTest(binary=binary.name):
                ...
                external_h4096 = _matching_symbol_lines(
                    external_symbols,
                    "BuildPhase4CanonicalTrialSpecForCorpusV2H4096",
                )
                self.assertFalse(
                    external_h4096,
                    "H4096 builder unexpectedly has external linkage:\n"
                    + "\n".join(external_h4096[:20]),
                )
                for forbidden in (
                    "AllocateMultiWorld",
                    "AllocateOneWorld",
