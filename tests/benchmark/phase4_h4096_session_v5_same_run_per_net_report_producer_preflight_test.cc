...
#include "src/benchmark/phase4_confirmatory_h4096_session_v5_same_run_per_net_report_producer_preflight_internal.h"
#include "tests/support/google_test.h"

namespace apgar::benchmark {
namespace {

using Identity = internal::Phase4H4096SessionV5SameRunPerNetReportProducerIdentity;
...
void ExpectProducerIdentityError(const Identity& identity,
  ...
  ExpectInvariant(internal::PreflightPhase4ConfirmatoryH4096SessionV5SameRunPerNetReportProducer(
                      identity, source),
                  Phase4PairedTrialErrorCode::kMeasurementAssociation,
                  internal::kPhase4H4096SessionV5SameRunPerNetReportProducerInvariant);
}

TEST(Phase4H4096SessionV5SameRunPerNetReportProducerPreflightTest,
     CanonicalBuilderBindsEveryFrozenIdentityFieldExactly) {
  const Identity identity =
      internal::BuildPhase4ConfirmatoryH4096SessionV5SameRunPerNetReportProducerIdentity();
  ...
  EXPECT_EQ(identity.raw_authority, "phase4_confirmatory_same_run_raw_evidence_v3");
  EXPECT_EQ(identity.superseded_raw_authority, "phase4_confirmatory_same_run_raw_evidence_v2");
  EXPECT_EQ(identity.retained_raw_predecessor_authority,
            "phase4_confirmatory_same_run_raw_evidence_v1");
  EXPECT_TRUE(identity.telemetry_required);
  EXPECT_EQ(identity.telemetry_authority, "phase4_confirmatory_same_run_decision_telemetry_v3");
  EXPECT_EQ(identity.superseded_telemetry_authority,
            "phase4_confirmatory_same_run_decision_telemetry_v2");
  EXPECT_EQ(identity.retained_telemetry_predecessor_authority,
            "phase4_confirmatory_same_run_decision_telemetry_v1");
  EXPECT_EQ(identity.report_authority,
  ...
  EXPECT_EQ(identity.case_id, 10'100U);
  EXPECT_EQ(identity.requested_pool_size, 4U);
  EXPECT_EQ(identity.preparation_worker_count, 4U);
  EXPECT_EQ(identity.repetitions, 20U);
  EXPECT_EQ(identity.carrier, internal::Phase4H4096SessionV5Carrier::kSameRun);
  EXPECT_EQ(identity.raw_schema_version, 2U);
  EXPECT_EQ(identity.raw_wire_schema_version, 2U);
  EXPECT_EQ(identity.raw_pair_attempt_count, 20U);
  EXPECT_EQ(identity.raw_arm_attempt_count, 40U);
  EXPECT_EQ(identity.telemetry_schema_version, 1U);
  EXPECT_EQ(identity.telemetry_wire_schema_version, 2U);
  EXPECT_EQ(identity.telemetry_pair_capture_count, 20U);
  EXPECT_EQ(identity.telemetry_arm_capture_count, 40U);
  EXPECT_EQ(identity.telemetry_workload_net_count_per_arm, 6U);
  EXPECT_EQ(identity.telemetry_total_per_net_row_count, 240U);
  EXPECT_EQ(identity.report_schema_version, 1U);
  EXPECT_EQ(identity.report_raw_wire_schema_version, 2U);
  ...
  EXPECT_EQ(identity.workload_net_count_per_arm, 6U);
  EXPECT_EQ(identity.total_report_per_net_row_count, 12U);
  EXPECT_EQ(identity.workload_net_roster_checksum, 12'521'697'377'381'992'336ULL);
  ...
  EXPECT_EQ(identity.canonical_algorithm_budget_checksum, 13'645'569'624'513'409'309ULL);
  EXPECT_EQ(identity.paired_semantic_budget_checksum, 12'493'092'620'111'240'227ULL);
}
...
constexpr std::array<IdentityMutation, 61> kIdentityMutations = {{
    ...
    {"retained-telemetry-predecessor-authority",
     +[](Identity* value) { value->retained_telemetry_predecessor_authority = "mutated"; }},
    ...
    {"carrier",
     +[](Identity* value) { value->carrier = internal::Phase4H4096SessionV5Carrier::kOrdinary; }},
    ...
    {"raw-pair-attempt-count", +[](Identity* value) { ++value->raw_pair_attempt_count; }},
    {"raw-arm-attempt-count", +[](Identity* value) { ++value->raw_arm_attempt_count; }},
    ...
    {"telemetry-pair-capture-count",
     +[](Identity* value) { ++value->telemetry_pair_capture_count; }},
    {"telemetry-arm-capture-count", +[](Identity* value) { ++value->telemetry_arm_capture_count; }},
    {"telemetry-workload-net-count-per-arm",
     +[](Identity* value) { ++value->telemetry_workload_net_count_per_arm; }},
    {"telemetry-total-per-net-row-count",
     +[](Identity* value) { ++value->telemetry_total_per_net_row_count; }},
...
static_assert(kIdentityMutations.size() == 61);
...
constexpr std::array<ProfileMutation, 10> kForeignProfiles = {{
    {"ordinary-session-v5-complete-artifact-and-budget-shape",
     +[](Identity* value) {
       value->raw_authority = "phase4_confirmatory_raw_evidence_v3";
       value->superseded_raw_authority = "phase4_confirmatory_raw_evidence_v2";
       value->retained_raw_predecessor_authority = "phase4_confirmatory_raw_evidence_v1";
       value->telemetry_required = false;
       value->telemetry_authority = "";
       value->superseded_telemetry_authority = "";
       value->retained_telemetry_predecessor_authority = "";
       value->report_authority = "phase4_confirmatory_per_net_report_publication_join_v3";
       ...
       value->case_id = 10'200;
       value->requested_pool_size = 8;
       value->carrier = internal::Phase4H4096SessionV5Carrier::kOrdinary;
       value->raw_schema_version = 1;
       value->raw_wire_schema_version = 1;
       value->raw_pair_attempt_count = 0;
       value->raw_arm_attempt_count = 0;
       value->telemetry_schema_version = 0;
       value->telemetry_wire_schema_version = 0;
       value->telemetry_pair_capture_count = 0;
       value->telemetry_arm_capture_count = 0;
       value->telemetry_workload_net_count_per_arm = 0;
       value->telemetry_total_per_net_row_count = 0;
       value->report_raw_wire_schema_version = 1;
       value->workload_net_count_per_arm = 64;
       value->total_report_per_net_row_count = 128;
       value->workload_net_roster_checksum = 718'781'758'134'362'332ULL;
       value->canonical_algorithm_budget_checksum = 7'657'176'792'159'702'821ULL;
       value->paired_semantic_budget_checksum = 13'340'538'727'848'385'478ULL;
     }},
    {"session-v4-protocol-v2-roster-v3-predecessor",
     +[](Identity* value) {
       ...
       value->raw_authority = "phase4_confirmatory_same_run_raw_evidence_v2";
       value->telemetry_authority = "phase4_confirmatory_same_run_decision_telemetry_v2";
       value->report_authority = "phase4_confirmatory_same_run_per_net_report_publication_join_v2";
       ...
       value->canonical_algorithm_budget_checksum = 8'829'615'304'625'888'656ULL;
       value->paired_semantic_budget_checksum = 5'851'813'264'365'595'594ULL;
     }},
    {"retained-v1-artifact-authorities-promoted",
     +[](Identity* value) {
       value->raw_authority = "phase4_confirmatory_same_run_raw_evidence_v1";
       value->telemetry_authority = "phase4_confirmatory_same_run_decision_telemetry_v1";
       value->report_authority = "phase4_confirmatory_same_run_per_net_report_publication_join_v1";
     }},
    {"predecessor-session-v4-budget-pair",
     +[](Identity* value) {
       value->canonical_algorithm_budget_checksum = 8'829'615'304'625'888'656ULL;
       value->paired_semantic_budget_checksum = 5'851'813'264'365'595'594ULL;
     }},
    {"ordinary-session-v5-budget-pair",
     +[](Identity* value) {
       value->canonical_algorithm_budget_checksum = 7'657'176'792'159'702'821ULL;
       value->paired_semantic_budget_checksum = 13'340'538'727'848'385'478ULL;
     }},
    {"same-run-successor-budget-checksums-swapped",
     +[](Identity* value) {
    ...
    {"raw-cardinalities-normalized-from-one-another",
     +[](Identity* value) {
       value->raw_pair_attempt_count = 40;
       value->raw_arm_attempt_count = 20;
     }},
    {"telemetry-cardinalities-normalized-from-report",
     +[](Identity* value) {
       value->telemetry_pair_capture_count = 2;
       value->telemetry_arm_capture_count = 12;
       value->telemetry_workload_net_count_per_arm = 12;
       value->telemetry_total_per_net_row_count = 12;
     }},
    {"report-cardinalities-normalized-from-telemetry",
     +[](Identity* value) {
       value->report_arm_count = 40;
       value->workload_net_count_per_arm = 240;
       value->total_report_per_net_row_count = 240;
     }},
    {"absent-telemetry-companion",
     +[](Identity* value) {
       value->telemetry_required = false;
       value->telemetry_authority = "";
       value->superseded_telemetry_authority = "";
...
TEST(Phase4H4096SessionV5SameRunPerNetReportProducerPreflightTest,
  ...
  ExpectInvariant(internal::PreflightPhase4ConfirmatoryH4096SessionV5SameRunPerNetReportProducer(
                  ...
                  Phase4PairedTrialErrorCode::kUnsupportedSchema,
