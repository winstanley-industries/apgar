"""Acquisition-free process tests for the closed H2250 same-run report runner."""
...
class Phase4ConfirmatorySameRunPerNetReportProcessTest(unittest.TestCase):
    ...
    def test_both_report_authorities_reject_foreign_scope_before_stdout(self) -> None:
        same_run = report_command(
            "phase4_confirmatory_same_run_per_net_report_test_runner",
            testing=True,
        )
        mutations = (
            [argument for argument in same_run if not argument.startswith("--corpus_version=")],
            replace(same_run, "corpus_version", "1"),
            replace(same_run, "raw_wire_schema_version", "1"),
            replace(same_run, "pool_size", "8"),
            replace(same_run, "case_id", "10101"),
            replace(same_run, "case_id", "10200"),
            replace(same_run, "candidate_arm_artifact_checksum", "0"),
            same_run + [same_run[-1]],
            same_run + ["--unknown=1"],
        )
    ...
    def test_publication_validator_opens_raw_before_sidecar_and_report(self) -> None:
        raw_fifo = self.root / "raw.fifo"
        sidecar_fifo = self.root / "sidecar.fifo"
        report_fifo = self.root / "report.fifo"
        for path in (raw_fifo, sidecar_fifo, report_fifo):
            os.mkfifo(path)
        ...
        self.assertNotIn("same-run telemetry must be a regular file", invalid_raw.stderr)
        self.assertNotIn("per-net report must be a regular file", invalid_raw.stderr)
