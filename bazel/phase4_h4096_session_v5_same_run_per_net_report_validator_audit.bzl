"""Audits the Session-v5 H4096 same-run per-net validator runfiles closure."""
...
_ALLOWED_APGAR_RUNFILES = (
    ...
    "tools/validate_phase4_confirmatory_h4096_session_v5_same_run_per_net_report.py",
    ...
    "tools/validate_phase4_same_run_decision_telemetry.py",
...
def _audit_runfile(file, kind):
    ...
    if _is_generated_validator_runtime(file):
        if file.owner != _VALIDATOR:
            fail(
                "the Session-v5 same-run per-net validator audit observed substituted " +
    ...
    fail(
        "the Session-v5 same-run per-net validator %s runfiles contain an unexpected " +
...
def _audit_runfiles(runfiles, kind):
    ...
    if link_kind != None:
        fail(
            "the Session-v5 same-run per-net validator %s runfiles contain a " +
            "forbidden %s entry" % (kind, link_kind),
        )
    observed_apgar_runfiles = {}
    for file in runfiles.files.to_list():
        _audit_runfile(file, kind)
        if file.short_path in _ALLOWED_APGAR_RUNFILES:
            observed_apgar_runfiles[file.short_path] = True
    missing = [
        path
        for path in _ALLOWED_APGAR_RUNFILES
        if path not in observed_apgar_runfiles
    ]
    if missing:
        fail(
            "the Session-v5 same-run per-net validator %s runfiles omit exact " +
            "allowlisted APGAR paths: %s" % (kind, ", ".join(missing)),
        )

def _phase4_h4096_session_v5_same_run_per_net_report_validator_audit_impl(ctx):
    ...
    if executable == None or executable not in files:
        fail("the Session-v5 same-run per-net validator audit requires one exact executable")
    if executable.short_path != _VALIDATOR_PATH:
        fail("the Session-v5 same-run per-net validator audit observed path substitution")
    if executable.owner != _VALIDATOR:
        fail("the Session-v5 same-run per-net validator audit observed owner substitution")
    ...
    if executable not in default_runfiles:
        fail("the Session-v5 same-run per-net validator audit omitted its executable from runfiles")
...
_phase4_h4096_session_v5_same_run_per_net_report_validator_audit = rule(
    implementation = _phase4_h4096_session_v5_same_run_per_net_report_validator_audit_impl,
    attrs = {
        "_validator": attr.label(default = _VALIDATOR),
    },
)

def phase4_h4096_session_v5_same_run_per_net_report_validator_audit(name):
    _phase4_h4096_session_v5_same_run_per_net_report_validator_audit(
        name = name,
        testonly = True,
        visibility = ["//visibility:private"],
    )

def _phase4_h4096_session_v5_same_run_per_net_report_validator_audit_negative_contract_impl(
        ctx):
...
_phase4_h4096_session_v5_same_run_per_net_report_validator_audit_negative_contract = rule(
    implementation = (
        _phase4_h4096_session_v5_same_run_per_net_report_validator_audit_negative_contract_impl
    ),
)

def phase4_h4096_session_v5_same_run_per_net_report_validator_audit_negative_contract(name):
    _phase4_h4096_session_v5_same_run_per_net_report_validator_audit_negative_contract(
        name = name,
        testonly = True,
        visibility = ["//visibility:private"],
    )
