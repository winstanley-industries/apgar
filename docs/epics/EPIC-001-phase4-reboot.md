...
| P4R-02A2 | Bind prepared-context identity through CPU/GPU producer evidence, route admission, and exact immutable-candidate admission. | P4R-02A1 | Distinct-net requests produce and exactly admit correctly attributed immutable candidates; durable CPU, GPU, candidate, device, and replay association tokens carry retained-profile identity including routed net without reinterpreting the V1 numeric-rule scalar (use a separately versioned identity or an `APGAR-M1-RULE-BUCKET-V2` tag plus compiled-board/replay schema bumps); exact candidate self-clearance and terminal validation consume the CompiledBoard's retained routing profile rather than the Board IR default; relabeled or mismatched context evidence fails closed with context-neutral detail. | `ready` | — |
