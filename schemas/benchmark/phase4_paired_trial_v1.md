...
V1 regeneration draft = draft(X, B)
V2 regeneration draft = draft(X, B+1)
```
...
The V2 `+1` reserves the hard-ban policy entry that becomes reachable when
`maximum_columns_per_net` is two. V2 retains exactly `C=N` total columns per
epoch, so the query/work opportunity equations above do not change. Its
calibration-only canonical spec uses equal-arm negotiated-price steps
`present=1` and `history=2250`; the V1 budget roster remains byte-for-byte
unchanged.
...
exercise these current diagnostic joins.
