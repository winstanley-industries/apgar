...
target-specific standalone Bazel runfiles tree. Public launchers clear ambient
...
publication path. In a containing Bazel test, an invocation path may traverse
an enclosing runfiles symlink only when it resolves to the canonical launcher;
the enclosing tree is never an execution-authority candidate. The entire
standalone runfiles root, including every external repository subtree, must be
traversed completely before delegation. Direct stage-one Python execution must
disable `site` initialization until the rules_python bootstrap establishes the
authenticated standalone root; stage-two site initialization may run only
