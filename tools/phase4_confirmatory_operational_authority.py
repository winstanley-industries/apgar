"""Closed authority constants for the ordinary Corpus-v2 operational slice."""
...
PRODUCTION_WORKER = "phase4_confirmatory_operational_replay_worker"
TEST_WORKER = "phase4_confirmatory_operational_replay_test_worker"
PRODUCTION_INVOCATION = (
    "bazel --batch run --config=benchmark //:phase4_confirmatory_operational_capture"
)
PRODUCTION_WORKER_TARGET = "//:phase4_confirmatory_operational_replay_worker"
TEST_INVOCATION = (
    "bazel --batch run --config=benchmark //:phase4_confirmatory_operational_capture_test"
)
TEST_WORKER_TARGET = "//:phase4_confirmatory_operational_replay_test_worker"
...
def resolve_bundled_worker(relative: str) -> pathlib.Path:
    ...
    if runfiles_root is None:
        raise OSError("confirmatory operational authority requires its Bazel runfiles tree")
    candidate = runfiles_root / relative
    if not candidate.is_file():
        raise OSError(f"bundled confirmatory operational worker is unavailable: {relative}")
    return candidate
